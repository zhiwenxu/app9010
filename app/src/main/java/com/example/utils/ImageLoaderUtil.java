...
//		.decodingOptions(android.graphics.BitmapFactory.Options decodingOptions)//设置图片的解码配置
	...
	public DisplayImageOptions getBaseOptions(int image,boolean cacheInMemory){
		DisplayImageOptions options;
		options = new DisplayImageOptions.Builder()
				.showImageOnLoading(image) //设置图片在下载期间显示的图片
				.showImageForEmptyUri(image)//设置图片Uri为空或是错误的时候显示的图片
				.showImageOnFail(image)  //设置图片加载/解码过程中错误时候显示的图片
